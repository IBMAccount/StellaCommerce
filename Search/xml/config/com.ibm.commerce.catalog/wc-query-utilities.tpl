...
BEGIN_SQL_STATEMENT
	...
	sql=
    select distinct ADSC.SRCHFIELDNAME, AVD.value, AV.storeent_id, AVD.sequence, AVD.image1, AVD.image2, AVD.field1, AVD.field2, AVD.field3,QTD.qtyunit_id, QTD.description  
	from 
	   attrdictsrchconf ADSC
	   ...
	   left outer join qtyunitdsc QTD on QTD.qtyunit_id=AVD.qtyunit_id and QTD.language_id=?languageId?
	where 
...
BEGIN_SQL_STATEMENT
	...
	select distinct ADSC.SRCHFIELDNAME, AVD.value, AV.storeent_id, AVD.sequence, AVD.image1, AVD.image2, AVD.field1, AVD.field2, AVD.field3 ,QTD.qtyunit_id, QTD.description 
	from 
	   $SCHEMA$.attrdictsrchconf ADSC
	   inner join $SCHEMA$.attr A on ADSC.attr_id = A.attr_id
	   inner join $SCHEMA$.attrval AV on AV.attr_id = A.attr_id
	   inner join $SCHEMA$.attrvaldesc AVD on AV.attrval_id = AVD.attrval_id 
	   left outer join $SCHEMA$.qtyunitdsc QTD on QTD.qtyunit_id=AVD.qtyunit_id and QTD.language_id=?languageId?
	where 
	  ADSC.ATTR_ID is not NULL and  
	  ADSC.mastercatalog_id = ?catalogId? and
	  ADSC.srchfieldname in (?searchFieldName?) and
...
<!-- ========================================================================= -->
<!--  Get category facet sort order from FACET table                           -->
<!-- ========================================================================= -->
BEGIN_SQL_STATEMENT
	name=SELECT_CATEGORY_FACET_SORT_ORDER
	base_table=FACET
	sql=
	SELECT SORT_ORDER FROM FACET WHERE FACET_ID=-1001
END_SQL_STATEMENT
