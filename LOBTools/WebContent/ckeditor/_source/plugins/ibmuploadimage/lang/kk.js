/*
 * Copyright (c) 2003-2014, CKSource - Frederico Knabben. All rights reserved.
 * For licensing, see LICENSE.md or http://ckeditor.com/licensePortions Copyright IBM Corp., 2009-2015.
 */
CKEDITOR.plugins.setLang( 'ibmuploadimage', 'kk', {
	alertUrl	: "Сурет URL мекенжайын жазыңыз",
	alt		: "Балама мәтін:",
	border	: "Жиек:",
	btnUpload	: "Кескінді жүктеп салу",
	button2Img	: "Бөлектелген сурет түймешігін қарапайым суретке айналдыру керек пе?",
	hSpace	: "Көлденең орын:",
	img2Button	: "Бөлектелген суретті сурет түймешігіне айналдыру керек пе?",
	infoTab	: "Сурет сипаттары",
	linkTab	: "Сілтеме",
	lockRatio	: "Коэффициентті құлыптау",
	menu		: "Сурет сипаттары",
	resetSize	: "Өлшемді қайта орнату",
	title		: "Сурет кірістіру",
	titleButton	: "Сурет түймешігінің сипаттары",
	upload	: "Жүктеп салу",
	urlMissing : "Сурет көзінің URL мекенжайы жоқ.",
	vSpace	: "Тік орын:",
	validateBorder : "Жиек оң бүтін сан болуы қажет.",
	validateHSpace : "Көлденең аралығы оң бүтін сан болуы қажет.",
	validateVSpace : "Тік аралығы оң бүтін сан болуы қажет.",
	
	ibm : 
	{
        insertButtonLabel : "Кірістіру",
        insertButtonTitle : "Сурет кірістіру",
		previewText : "Мәтін осы мысалдағыдай қосылудағы суреттің айналасында болады.",
        imageLinkLabel : "Кескін басылған кезде ашылатын сілтеме қосыңыз.",
		fileUpload : "Кескін файлын компьютеріңізден таңдаңыз:",
        browseFile : "Файлды таңдау",
        noFileChosen : "Ешбір файл таңдалмады",
        notImageFile: "Сурет файлын таңдаңыз.",
        fileUploadTimeout : "Кескінді енгізу өте ұзақ уақыт алды да, әрекет кідіртілді. Қайталап көріңіз немесе болдырмаңыз.",
	serverUnreachable : "Сервер ағым бойынша қол жетімсіз. Жүйе әкімшілігіне хабарласып, әрекетті кейін қайталап көріңіз.",
        linkLabel:"Сілтеме:",
        imageSize: "Кескін тым үлкен. Басқа кескін таңдаңыз немесе болдырмаңыз.",
        inserting: "Кірістіру...",
        enableActiveX: "Internet Explorer шолғышына ActiveX қосылып тұрғандығын мұқият тексеріңіз.",
        file: "Файл"
	}
});

