...
define([
	...
	return new declare(null, {
		...
		constructor: function() {
			...
			on(document, (!has("mozilla")? "mousewheel" : "DOMMouseScroll"), function(e) {
				var target = e.target;
				...
				if (mainContent){
					...
					EventUtil.trigger(KeyUtil, "onmousewheeldelta", (!has("mozilla")? e.wheelDelta : -e.detail));
				}
			});
		},
			...
			for (var i = 0; i < keys.length; i++) {
				...
				if (i == keys.length - 1) {
					if (typeof keyMapping.handlers != "undefined") {
						for (var j = 0; j < keyMapping.handlers.length; j++) {
