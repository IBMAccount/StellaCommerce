...
//				for (var i = 0; i < executableAgents.length; i++) {
//					executableAgents[i].locked = false;
//				}
			}
		...
		disconnectAll: function(context, method, scope) {
			...
			if (agent != null) {
				for (var i = 0; i < agent.events.length; i++) {
					this._disconnectEvents(agent.events[i].source, agent.events[i].name, agent);
				}
//				var obj = null;
//				if (agent.scope != null) {
//					obj = agent.scope;
...
//					if (obj.__cmcAgents[i] == agent) {
//						agent.context = null;
//						agent.method = null;
//						agent.scope = null;
//						agent.events = null;
//						obj.__cmcAgents.splice(i, 1);
//						break;
//					}
//				}
			}
