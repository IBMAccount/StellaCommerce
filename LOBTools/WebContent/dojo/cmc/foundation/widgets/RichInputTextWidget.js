...
define([
	...
	return declare(ComponentWidget, {
			...
			if (this._ckeditor){
				if (this._ckeditor.mode != 'source'){
					...
					if (doc){
						...
						height = Math.max(
							Math.max(scrollable.offsetHeight, body.offsetHeight),
							Math.max(scrollable.scrollHeight, body.scrollHeight),
							Math.max(scrollable.clientHeight, body.clientHeight)
							) + (
								Math.max(
								Math.max(scrollable.offsetWidth, body.offsetWidth),
								Math.max(scrollable.scrollWidth, body.scrollWidth),
								Math.max(scrollable.clientWidth, body.clientWidth))
		...
		removeCKEditor: function(){
			...
			if (this.component.getRichTextMode()){
				this._ckeditor.destroy();
				this._ckeditor = null;
				this.swithModeHandler.remove();
				this.swithModeHandler = null;
